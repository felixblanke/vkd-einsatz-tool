...
public class ComparatorChain<E> implements Comparator<E> {
  private final List<NamedComparator<E>> comparatorChain = new ArrayList<>();
  ...
  /**
   * Clones the underlying {@code List<NamedComparator>} used to implement the {@code
   * ComparatorChain} and returns the clone.
   *
   * @return A clone of the internal {@code ComparatorChain}
   */
  public List<NamedComparator<E>> getComparatorChain() {
    return new ArrayList<>(comparatorChain);
  }
}
