...
public class Framework<E> {
  ...
  private final Map<String, String> stringValMap = new HashMap<>();
  ...
  /**
   * Sorts a List using a {@code ComparatorChain}. If comparatorChain is null, the original list is
   * returned
   *
   * @param unsortedList    The List that should be sorted
   * @param comparatorChain The Chain that dictates the order used to sort the List
  ...
  public static <T> List<T> sort(List<T> unsortedList, ComparatorChain<T> comparatorChain) {
    List<T> temp = new ArrayList<>(unsortedList);
    if (comparatorChain == null) {
      return temp;
    }
    temp.sort(comparatorChain);
    return temp;
  }
      ...
      public String format(LogRecord record) {
        ...
        if (printStackTrace) {
          for (StackTraceElement st : record.getThrown().getStackTrace()) {
            sb.append("\tat ").append(st.getClassName()).append(".").append(st.getMethodName())
  ...
  /**
   * Loads the Strings into {@code enumMap} to be used in Main#getString(String)
   *
  ...
  public void loadStringValuesFromXML(String filedir, boolean loadFromJar) throws Exception {
    ...
    List<Variable> localVarList = new ArrayList<>();
    ...
    for (Element v : lang.getChildren(this.varSet.get("xml_var"))) {
      ...
      boolean b = false;
      for (Variable temp : localVarList) {
      ...
      if (!b) {
        ...
        if (temp != null) {
          ...
          localVarList.add(new Variable(varName, varValue));
        }
      }
    }
    ...
    for (Element v : lang.getChildren(this.varSet.get("xml_value"))) {
      ...
      valText = replaceVars(valText, localVarList, this.varSet.get("xml_localVar_marker"),
          this.varSet.get("xml_func_arg_splitter"));
  ...
  /**
   * The loading of the Strings from the XML-File supports a very basic kind of variables and
   * "functions". The variables are simple String stored in the XML-File itself and referred to in
   * the entries which deFINER a specific String. To refer to this variable in the String, one
   * should sourround the variable name with the deFINERd variableMarker, e.g. {@code ?EL?} (? as
   * the variableMarker) The methods are basically a scheme with some placeholders and the args
   * (only Strings) are set in place of the placeholders. This code checks if the current String
   * uses a variable using {@code vars} and checks then, if the variable has args attached to it by
   * brackets. If either is true, the variable/method is replaced accordingly and the method checks
   * for the next variable/method. If each known variable/method is checked, the String is
   * returned.
   *
   * @param input          A String read from the XML-File. It could contain variables/methods
   *                       potentially.
   * @param vars           A List of all known variables/methods
   * @param variableMarker A marker which signifies the begin/end of a variable/method, e.g. {@code
   *                       '?'} in {@code ?EL?}
   * @param argSplitter    A marker which is used to differentiate between multiple arguments, e.g.
   *                       {@code ','} in {@code ?METHOD?(ARG0,ARG1,ARG2)}
   * @return The processed input
   */
  private String replaceVars(String input, List<Variable> vars, String variableMarker,
                             String argSplitter) {
    if (vars == null) {
      return input;
    }
            ...
            if (c == '(') {
              if (input.indexOf(')', index + varLength) > -1) {
                ...
                String bracketContent =
                    input.substring(index + varLength + 1, input.indexOf(')', index + varLength));
              ...
              if (index + varLength + 1 < input.length()) {
                if (input.charAt(index + varLength + 1) == '(') {
                  input = input.substring(0, index + varLength + 1)
                      .concat(input.substring(index + varLength + 2));
                }
              }
            }
          }
  ...
  /**
   * Returns the String belonging to the String value.
   *
   * @param stringName The StringValue, whose  String should be returned
   * @return The String belonging to the StringValue. If the StringValue has no String, null is
   *     returned instead.
   */
