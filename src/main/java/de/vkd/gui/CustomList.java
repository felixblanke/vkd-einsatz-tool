...
/**
 * @param <E> The Elements of the List, e.g. VK or Kuerzung
 * @author Felix Blanke Only used in {@link SortDialog}
 */

@SuppressWarnings("serial")
public class CustomList<E> extends JList<String> {
  private final ComparatorListModel model;
  ...
  class ComparatorListModel extends AbstractListModel<String> {
    private final List<NamedComparator<E>> currentList;

    public ComparatorListModel(List<NamedComparator<E>> currentList) {
      this.currentList = currentList;
        if (currentList != null && !currentList.isEmpty()) {
            super.fireIntervalAdded(this, 0, currentList.size() - 1);
        }
    }
